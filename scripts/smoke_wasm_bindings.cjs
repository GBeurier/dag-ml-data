...
const pkgDir = path.resolve(
  process.argv[2] || path.join(repo, "target", "wasm", "dag-ml-data-wasm"),
);
...
try {
  ...
  if (descriptor.category !== "data" || descriptor.code !== "data_contract_validation") {
    throw new Error("WASM error descriptor taxonomy drifted");
  }
  if (!descriptor.message.includes("shape contract")) {
    throw error;
  }
}
