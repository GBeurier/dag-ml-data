...
| Feature buffers | typed numeric buffer projection, duplicate feature/observation/column refusal, row-major f64 matrix shape/mask validation, deterministic manifests and fingerprints, store-level duplicate feature-set refusal, core arena bind/project/release lifecycle, source/relation coverage bindings for materialized handles |
